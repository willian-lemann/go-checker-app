...
import { Loader2, CheckCircle2, Search, FileText, Link2, Shield, Zap, Code } from "lucide-react"
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 p-3 rounded-lg transition-all duration-300 ${
                isCompleted ? "bg-emerald-400/10" : isCurrent ? "bg-primary/10" : "bg-secondary/50 opacity-50"
              }`}
            >
              <div
                className={`w-8 h-8 rounded-full flex items-center justify-center ${
                  isCompleted ? "bg-emerald-400/20" : isCurrent ? "bg-primary/20" : "bg-secondary"
                }`}
              >
                {isCompleted ? (
